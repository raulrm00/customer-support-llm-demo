...
def test_client():
    ...
    user = User(
        ...
        daily_usage_reset_at=datetime.now(UTC),
    )
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_usage_limit_exceeded(test_client):
    ...
    def override_service():
        return PredictionService(
            bundle=ModelBundle(
                pipeline=FakePipeline(), model_version="v1", pipeline_version="v1"
            )
        )
...
def test_usage_limit_reset(test_client):
    ...
    user.daily_usage_reset_at = datetime.now(UTC) - timedelta(hours=25)
    db.commit()
    ...
    db.refresh(user)
    assert (
        user.daily_usage_seconds < 1.0
    )  # It will be slightly > 0 due to processing time of this request
